...
function setTempHome(base: string) {
  ...
  process.env.OPENCLAW_STATE_DIR = path.join(base, ".bobolt");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "bobolt-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".bobolt", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
