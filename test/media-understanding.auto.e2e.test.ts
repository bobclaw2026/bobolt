...
import type { BoboltConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("bobolt-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("bobolt-bin-sherpa-");
      const modelDir = await makeTempDir("bobolt-sherpa-model-");
      ...
      const cfg: BoboltConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("bobolt-bin-whispercpp-");
      const modelDir = await makeTempDir("bobolt-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("bobolt-bin-gemini-");
      ...
      const cfg: BoboltConfig = { tools: { media: { image: {} } } };
