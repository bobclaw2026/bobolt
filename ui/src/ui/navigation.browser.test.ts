...
import { BoboltApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = BoboltApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("bobolt-app") as BoboltApp;
...
afterEach(() => {
  BoboltApp.prototype.connect = originalConnect;
  window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/bobolt/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/bobolt");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/bobolt/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = "/bobolt";
    const app = mountApp("/bobolt/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/bobolt");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/bobolt/sessions");
  });
