...
This directory contains hooks that ship with Bobolt. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.bobolt.ai/hooks/soul-evil
...
Hooks can be configured in `~/.bobolt/bobolt.json`:
...
2. Restart gateway: `pkill -9 -f 'bobolt.*gateway' && pnpm bobolt gateway`
3. Enable the hook: `bobolt hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.bobolt.ai/hooks
