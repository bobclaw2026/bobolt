...
homepage: https://docs.bobolt.ai/hooks#command-logger
metadata:
  {
    "bobolt":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.bobolt/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.bobolt/logs/commands.log ~/.bobolt/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/bobolt`:
   ```
   /home/username/.bobolt/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
