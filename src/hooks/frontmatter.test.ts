...
`;
    ...
    expect(parsed.bobolt.emoji).toBe("💾");
    expect(parsed.bobolt.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.bobolt.emoji).toBe("📝");
    expect(parsed.bobolt.events).toEqual(["command"]);
    expect(parsed.bobolt.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.bobolt.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"bobolt": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"bobolt": {"events": ["test"]}}');
  });
...
describe("resolveBoboltMetadata", () => {
  it("extracts bobolt metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveBoboltMetadata(frontmatter);
  ...
  it("returns undefined when bobolt key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        bobolt: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Bobolt" },
            { id: "npm", kind: "npm", package: "@bobolt/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@bobolt/hook");
  });
    ...
    "bobolt":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Bobolt" }],
      },
  }
---
...
`;
    ...
    const bobolt = resolveBoboltMetadata(frontmatter);
    expect(bobolt).toBeDefined();
    expect(bobolt?.emoji).toBe("💾");
    expect(bobolt?.events).toEqual(["command:new"]);
    expect(bobolt?.requires?.config).toEqual(["workspace.dir"]);
    expect(bobolt?.install?.[0].kind).toBe("bundled");
  });
