...
import { createBoboltCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("bobolt-ws-", async (workspaceDir) => {
      await withTempDir("bobolt-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createBoboltCodingTools({ workspaceDir });
    ...
    await withTempDir("bobolt-ws-", async (workspaceDir) => {
      await withTempDir("bobolt-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello bobolt");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("bobolt-ws-", async (workspaceDir) => {
      const tools = createBoboltCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("bobolt-ws-", async (workspaceDir) => {
      await withTempDir("bobolt-override-", async (overrideDir) => {
        const tools = createBoboltCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("bobolt-sandbox-", async (sandboxDir) => {
      await withTempDir("bobolt-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createBoboltCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
