...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "bobolt.sandbox=1");
  args.push("--label", `bobolt.sessionKey=${params.scopeKey}`);
  args.push("--label", `bobolt.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `bobolt.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("bobolt.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`bobolt sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`bobolt sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("bobolt sandbox recreate --all");
}
