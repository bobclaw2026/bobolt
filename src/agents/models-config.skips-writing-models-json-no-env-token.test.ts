...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "bobolt-models-" });
}

const MODELS_CONFIG: BoboltConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureBoboltModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureBoboltModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureBoboltModelsJson } = await import("./models-config.js");
      const { resolveBoboltAgentDir } = await import("./agent-paths.js");

      await ensureBoboltModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveBoboltAgentDir(), "models.json");
