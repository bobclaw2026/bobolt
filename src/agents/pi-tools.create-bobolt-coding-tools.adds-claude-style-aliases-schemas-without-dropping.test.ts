...
import { createBoboltTools } from "./bobolt-tools.js";
import { __testing, createBoboltCodingTools } from "./pi-tools.js";
...
describe("createBoboltCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createBoboltCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "bobolt-outside.txt");
