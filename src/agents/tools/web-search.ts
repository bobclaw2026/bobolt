...
type WebSearchConfig = NonNullable<BoboltConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: BoboltConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("bobolt configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.bobolt.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://bobolt.ai",
      "X-Title": "Bobolt Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
