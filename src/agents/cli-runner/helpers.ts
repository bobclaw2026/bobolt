...
/**
 * Cleanup suspended Bobolt CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: BoboltConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-cli-images-"));
  const paths: string[] = [];
