...
describe("resolveTaskScriptPath", () => {
  it("uses default path when OPENCLAW_PROFILE is default", () => {
    ...
    expect(resolveTaskScriptPath(env)).toBe(
      path.join("C:\\Users\\test", ".bobolt", "gateway.cmd"),
    );
  });
  ...
  it("uses profile-specific path when OPENCLAW_PROFILE is set to a custom value", () => {
    ...
    expect(resolveTaskScriptPath(env)).toBe(
      path.join("C:\\Users\\test", ".bobolt-jbphoenix", "gateway.cmd"),
    );
  });
  ...
  it("prefers OPENCLAW_STATE_DIR over profile-derived defaults", () => {
    const env = {
      ...
      OPENCLAW_STATE_DIR: "C:\\State\\bobolt",
    };
    expect(resolveTaskScriptPath(env)).toBe(path.join("C:\\State\\bobolt", "gateway.cmd"));
  });
  ...
  it("trims whitespace from OPENCLAW_PROFILE", () => {
    ...
    expect(resolveTaskScriptPath(env)).toBe(
      path.join("C:\\Users\\test", ".bobolt-myprofile", "gateway.cmd"),
    );
  });

  it("falls back to HOME when USERPROFILE is not set", () => {
    const env = { HOME: "/home/test", OPENCLAW_PROFILE: "default" };
    expect(resolveTaskScriptPath(env)).toBe(path.join("/home/test", ".bobolt", "gateway.cmd"));
  });
});

describe("readScheduledTaskCommand", () => {
  it("parses basic command script", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-schtasks-test-"));
    try {
      const scriptPath = path.join(tmpDir, ".bobolt", "gateway.cmd");
  ...
  it("parses script with working directory", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        scriptPath,
        ["@echo off", "cd /d C:\\Projects\\bobolt", "node gateway.js"].join("\r\n"),
        "utf8",
      );
      ...
      expect(result).toEqual({
        ...
        workingDirectory: "C:\\Projects\\bobolt",
      });
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
