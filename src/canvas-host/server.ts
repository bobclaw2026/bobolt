...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.boboltCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.boboltCanvasA2UIAction &&
        typeof window.boboltCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.boboltSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("bobolt:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Bobolt node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.boboltSendUserAction === "function"
        ? window.boboltSendUserAction
        : undefined;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Bobolt Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
