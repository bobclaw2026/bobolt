...
function candidateBinDirs(opts: EnsureBoboltPathOpts): string[] {
  ...
  // Bundled macOS app: `bobolt` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "bobolt");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/bobolt` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "bobolt"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `bobolt` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureBoboltCliOnPath(opts: EnsureBoboltPathOpts = {}) {
