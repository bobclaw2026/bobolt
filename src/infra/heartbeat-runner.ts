...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: BoboltConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: BoboltConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: BoboltConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: BoboltConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: BoboltConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: BoboltConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: BoboltConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
