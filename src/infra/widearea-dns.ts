...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*bobolt-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "bobolt";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "bobolt");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "bobolt-gw");
  ...
  records.push(`_bobolt-gw._tcp IN PTR ${instanceLabel}._bobolt-gw._tcp`);
  records.push(`${instanceLabel}._bobolt-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._bobolt-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; bobolt-content-hash: ${contentHash}\n${contentBody}`;
}
