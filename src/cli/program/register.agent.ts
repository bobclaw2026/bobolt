...
${formatHelpExamples([
  ['bobolt agent --to +15555550123 --message "status update"', "Start a new session."],
  ['bobolt agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'bobolt agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'bobolt agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['bobolt agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.bobolt.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.bobolt.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['bobolt agents set-identity --agent main --name "Bobolt" --emoji "🦞"', "Set name + emoji."],
  ["bobolt agents set-identity --agent main --avatar avatars/bobolt.png", "Set avatar path."],
  [
    "bobolt agents set-identity --workspace ~/.bobolt/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "bobolt agents set-identity --identity-file ~/.bobolt/workspace/IDENTITY.md --agent main",
