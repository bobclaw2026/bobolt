...
import { resolveBoboltPackageRoot } from "../infra/bobolt-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "bobolt doctor entry": "Checking doctor entrypoint",
  "bobolt doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "bobolt";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OPENCLAW_REPO_URL = "https://github.com/bobolt/bobolt.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("bobolt@")) {
    return trimmed.slice("bobolt@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "bobolt.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("bobolt completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCLAW_GIT_DIR to an empty folder or an bobolt checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveBoboltPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Bobolt update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Bobolt..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Bobolt install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("bobolt doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("bobolt gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g bobolt@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g bobolt@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("bobolt gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("bobolt doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("bobolt gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("bobolt gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `bobolt update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENCLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENCLAW_GIT_DIR to an empty folder or an bobolt checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Bobolt to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["bobolt update", "Update a source checkout (git)"],
        ["bobolt update --channel beta", "Switch to beta channel (git + npm)"],
        ["bobolt update --channel dev", "Switch to dev channel (git + npm)"],
        ["bobolt update --tag beta", "One-off update to a dist-tag or version"],
        ["bobolt update --no-restart", "Update without restarting the service"],
        ["bobolt update --json", "Output result as JSON"],
        ["bobolt update --yes", "Non-interactive (accept downgrade prompts)"],
        ["bobolt update wizard", "Interactive update wizard"],
        ["bobolt --update", "Shorthand for bobolt update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run bobolt update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.bobolt.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["bobolt update status", "Show channel + version status."],
          ["bobolt update status --json", "JSON output."],
          ["bobolt update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.bobolt.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
