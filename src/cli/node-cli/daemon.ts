...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("bobolt node install"), formatCliCommand("bobolt node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("bobolt node install --force")}`);
    }
    return;
  }
