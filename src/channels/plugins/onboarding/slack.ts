...
function setSlackDmPolicy(cfg: BoboltConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Bobolt";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Bobolt`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: BoboltConfig, allowFrom: string[]): BoboltConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
