...
function summarizeGroupPolicy(cfg: BoboltConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function collectModels(cfg: BoboltConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function hasWebSearchKey(cfg: BoboltConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: BoboltConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: BoboltConfig): boolean {
...
function isBrowserEnabled(cfg: BoboltConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: BoboltConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: BoboltConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPENCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("bobolt security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: BoboltConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
...
export function collectHooksHardeningFindings(cfg: BoboltConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: BoboltConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: BoboltConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
