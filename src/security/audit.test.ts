...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "bobolt.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "bobolt.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: BoboltConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-discord-"));
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "bobolt-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.OPENCLAW_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.OPENCLAW_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-discord-open-"));
    process.env.OPENCLAW_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: BoboltConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.OPENCLAW_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-slack-"));
    process.env.OPENCLAW_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: BoboltConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.OPENCLAW_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-slack-open-"));
    process.env.OPENCLAW_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: BoboltConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.OPENCLAW_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-telegram-"));
    process.env.OPENCLAW_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: BoboltConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: BoboltConfig = { gateway: { mode: "local" } };
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.bobolt",
      configPath: "/Users/test/Dropbox/.bobolt/bobolt.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: BoboltConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "bobolt.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-audit-scanner-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        bobolt: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
