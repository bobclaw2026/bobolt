...
describe("gateway config.patch", () => {
  ...
  it("writes config, stores sentinel, and schedules restart", async () => {
    ...
    const sentinelPath = path.join(os.homedir(), ".bobolt", "restart-sentinel.json");
...
describe("gateway server sessions", () => {
  it("filters sessions by agentId", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-sessions-agents-"));
  ...
  it("resolves and patches main alias to default agent main key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-sessions-"));
