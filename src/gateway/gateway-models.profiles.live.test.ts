...
import type { BoboltConfig, ModelProviderConfig } from "../config/types.js";
import { resolveBoboltAgentDir } from "../agents/agent-paths.js";
...
import { ensureBoboltModelsJson } from "../agents/models-config.js";
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<BoboltConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveBoboltAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.bobolt-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveBoboltAgentDir();
  const sanitizedCfg: BoboltConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-live-"));
  const tempConfigPath = path.join(tempDir, "bobolt.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.OPENCLAW_CONFIG_PATH = tempConfigPath;

  await ensureBoboltModelsJson(nextCfg);
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Bobolt live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureBoboltModelsJson(cfg);
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.bobolt-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
