...
export function resolveHooksConfig(cfg: BoboltConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: BoboltConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-bobolt-token"] === "string"
      ? req.headers["x-bobolt-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
