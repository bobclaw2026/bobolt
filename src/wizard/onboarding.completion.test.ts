...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "bobolt",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/bobolt.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("bobolt");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "bobolt");
