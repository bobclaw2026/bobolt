...
const ETC_OPENCLAW_DIR = path.join(ROOT_DIR, "etc", "bobolt");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "bobolt.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcBoboltPath(...parts: string[]) {
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcBoboltPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "bobolt.json"))).toEqual({
      shared: true,
    });
  });
});
