...
import type { BoboltConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: BoboltConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: BoboltConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = BoboltSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as BoboltConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as BoboltConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as BoboltConfig)),
    ),
  };
}
