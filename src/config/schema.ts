...
import { BoboltSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof BoboltSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when Bobolt writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /bobolt).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.bobolt/memory/{agentId}.sqlite).",
  ...
  "memory.backend": 'Memory backend ("builtin" for Bobolt embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `bobolt plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.bobolt/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/bobolt",
  "gateway.controlUi.root": "dist/control-ui",
  "gateway.controlUi.allowedOrigins": "https://control.example.com",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/bobolt.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = BoboltSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "BoboltConfig";
