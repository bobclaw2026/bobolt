...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "bobolt-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".bobolt", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<BoboltConfig> = {},
): BoboltConfig {
  const base: BoboltConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "bobolt"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".bobolt", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
