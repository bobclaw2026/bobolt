...
import type { BoboltConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: BoboltConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: BoboltConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as BoboltConfig),
...
export function getCommandDetection(_cfg?: BoboltConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: BoboltConfig) {
