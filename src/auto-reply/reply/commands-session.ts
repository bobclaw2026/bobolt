...
import { scheduleGatewaySigusr1Restart, triggerBoboltRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Bobolt in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerBoboltRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Bobolt via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
