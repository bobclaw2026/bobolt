...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "bobolt-commands-"));
...
function buildParams(commandBody: string, cfg: BoboltConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
