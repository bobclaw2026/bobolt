...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "bobolt-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "bobolt-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "bobolt-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "bobolt-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.bobolt when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "bobolt-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENCLAW_HOME over HOME", () => {
    ...
    expect(resolveHomeDir()).toBe(path.resolve("/srv/bobolt-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENCLAW_HOME prefix when OPENCLAW_HOME is set", () => {
    ...
    expect(shortenHomePath(`${path.resolve("/srv/bobolt-home")}/.bobolt/bobolt.json`)).toBe(
      "$OPENCLAW_HOME/.bobolt/bobolt.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENCLAW_HOME replacement when OPENCLAW_HOME is set", () => {
    ...
    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/bobolt-home")}/.bobolt/bobolt.json`),
    ).toBe("config: $OPENCLAW_HOME/.bobolt/bobolt.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/bobolt")).toBe(path.resolve(os.homedir(), "bobolt"));
  });
  ...
  it("prefers OPENCLAW_HOME for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/bobolt")).toBe(path.resolve("/srv/bobolt-home", "bobolt"));
