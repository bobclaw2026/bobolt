...
import { discoverBoboltPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: BoboltPluginDefinition;
  register?: BoboltPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as BoboltPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as BoboltPluginDefinition;
...
export function loadBoboltPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverBoboltPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "bobolt/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: BoboltPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as BoboltPluginModule;
    } catch (err) {
