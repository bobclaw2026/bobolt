...
export function listAgentEntries(cfg: BoboltConfig): AgentEntry[] {
...
function resolveAgentName(cfg: BoboltConfig, agentId: string) {
...
function resolveAgentModel(cfg: BoboltConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: BoboltConfig): AgentSummary[] {
