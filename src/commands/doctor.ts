...
function resolveMode(cfg: BoboltConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveBoboltPackageRoot({
  ...
  let cfg: BoboltConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("bobolt configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("bobolt config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("bobolt setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("bobolt doctor --fix")}" to apply changes.`);
  }
