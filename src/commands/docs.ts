...
const SEARCH_TOOL = "https://docs.bobolt.ai/mcp.SearchBobolt";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.bobolt.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('bobolt docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.bobolt.ai/");
      runtime.log(`Search: ${formatCliCommand('bobolt docs "your query"')}`);
    }
    return;
  }
