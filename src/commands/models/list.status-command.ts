...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveBoboltAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("bobolt models auth setup-token")}\` or \`${formatCliCommand("bobolt configure")}\`.`
          : `Run \`${formatCliCommand("bobolt configure")}\` or set an API key env var.`;
