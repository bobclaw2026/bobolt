...
export async function updateConfig(
  mutator: (cfg: BoboltConfig) => BoboltConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: BoboltConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: BoboltConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("bobolt agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
