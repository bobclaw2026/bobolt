...
import { readConfigFileSnapshot, type BoboltConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: BoboltConfig, model: string): BoboltConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("bobolt plugins install")}\`.`,
    );
  }
