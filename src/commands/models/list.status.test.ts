...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveBoboltAgentDir: vi.fn().mockReturnValue("/tmp/bobolt-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/bobolt-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/bobolt-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveBoboltAgentDir: mocks.resolveBoboltAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveBoboltAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/bobolt-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/bobolt-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/bobolt-agent-custom");
