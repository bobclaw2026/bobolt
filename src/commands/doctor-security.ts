...
import type { BoboltConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: BoboltConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("bobolt security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("bobolt configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("bobolt config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("bobolt doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("bobolt config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
