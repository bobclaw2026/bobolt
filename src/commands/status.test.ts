...
vi.mock("../infra/bobolt-root.js", () => ({
  resolveBoboltPackageRoot: vi.fn().mockResolvedValue("/tmp/bobolt"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/bobolt/pnpm-lock.yaml",
      markerPath: "/tmp/bobolt/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Bobolt status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("bobolt status --all") ||
          l.includes("bobolt --profile isolated status --all") ||
          l.includes("bobolt status --all") ||
          l.includes("bobolt --profile isolated status --all"),
      ),
    ).toBe(true);
