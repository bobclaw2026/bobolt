...
vi.mock("./chrome.js", () => ({
  resolveBoboltUserDataDir: vi.fn(() => "/tmp/bobolt-test/bobolt/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveBoboltUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "bobolt-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveBoboltUserDataDir).mockReturnValue(userDataDir);
