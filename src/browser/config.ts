import type { BrowserConfig, BrowserProfileConfig, BoboltConfig } from "../config/config.js";
...
/**
 * Ensure the default "bobolt" profile exists in the profiles map.
 ...
 *
 * Note: this is an Bobolt browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for bobolt/bobolt CDP).
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "bobolt";
