...
Deploy Bobolt on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy Bobolt](https://northflank.com/stacks/deploy-bobolt) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy Bobolt now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the Bobolt template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the Bobolt service.
8. Open the public Bobolt URL and complete setup at `/setup`.
9. Open the Control UI at `/bobolt`.

## What you get

- Hosted Bobolt Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/bobolt`
