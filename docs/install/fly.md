---
title: Fly.io
description: Deploy Bobolt on Fly.io
---

# Fly.io Deployment

**Goal:** Bobolt Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-bobolt

# Create a persistent volume (1GB is usually enough)
fly volumes create bobolt_data --size 1 --region iad
```
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `bobolt.json` where they could be accidentally exposed or logged.
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/bobolt.json`.
...
Or visit `https://my-bobolt.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/bobolt.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/bobolt.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/bobolt.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/bobolt.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/bobolt.json"
```
...
```bash
# List current IPs
fly ips list -a my-bobolt

# Release public IPs
fly ips release <public-ipv4> -a my-bobolt
fly ips release <public-ipv6> -a my-bobolt
...
fly ips allocate-v6 --private -a my-bobolt
```
...
fly proxy 3000:3000 -a my-bobolt
