...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Bobolt
...
Running Bobolt via Node with `tsx` fails at startup with:

```
[bobolt] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `bobolt status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch bobolt.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node bobolt.mjs status` works on Node 25.
