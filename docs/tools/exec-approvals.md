...
- **gateway host** → `bobolt` process on the gateway machine
...
`~/.bobolt/exec-approvals.json` directly.

CLI: `bobolt approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
