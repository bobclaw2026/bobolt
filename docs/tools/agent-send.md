---
summary: "Direct `bobolt agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `bobolt agent` (direct agent runs)

`bobolt agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `bobolt message --target`).
...
```bash
bobolt agent --to +15555550123 --message "status update"
...
bobolt agent --to +15555550123 --message "Summon reply" --deliver
