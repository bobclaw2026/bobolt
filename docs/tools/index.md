---
summary: "Agent tool surface for Bobolt (browser, canvas, nodes, message, cron) replacing legacy `bobolt-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `bobolt-*` skills
title: "Tools"
---

# Tools (Bobolt)

Bobolt exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `bobolt-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `bobolt.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Bobolt logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:bobolt`: all built-in Bobolt tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`bobolt node run`).
...
- Requires a Brave API key (recommended: `bobolt configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Bobolt-managed browser.
...
- Quick smoke: `bobolt nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `bobolt gateway` restart in-place)
