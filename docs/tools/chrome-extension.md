---
summary: "Chrome extension: let Bobolt drive your existing Chrome tab"
read_when:
...
The Bobolt Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate bobolt-managed Chrome profile.
...
Bobolt then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Bobolt release (npm package) as static files. There is no separate “build” step.

After upgrading Bobolt:

- Re-run `bobolt browser extension install` to refresh the installed files under your Bobolt state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Bobolt ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Bobolt to control.
...
- `ON`: attached; Bobolt can drive that tab.
...
Debugging: `bobolt sandbox explain`
...
`bobolt browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `bobolt browser extension install` first to copy the extension to a stable location under your Bobolt state directory.
...
- **This is not isolated** like the dedicated bobolt-managed profile.
