...
- During heartbeat runs, Bobolt treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `bobolt sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Bobolt skips the heartbeat run entirely (no model call).
...
```bash
bobolt system event --text "Check for urgent follow-ups" --mode now
```
