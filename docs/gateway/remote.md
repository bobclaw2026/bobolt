...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Bobolt runs”).
...
- `bobolt health` and `bobolt status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `bobolt gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.
