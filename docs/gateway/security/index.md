...
## Quick check: `bobolt security audit`
...
```bash
bobolt security audit
bobolt security audit --deep
bobolt security audit --fix
```
...
- Tighten local perms (`~/.bobolt` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

Bobolt is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Bobolt also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.bobolt/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.bobolt/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.bobolt/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.bobolt/credentials/oauth.json`
...
`bobolt security audit` warns when this setting is enabled.
...
Bobolt stores session transcripts on disk under `~/.bobolt/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.bobolt` (see the audit section below). If you need
...
Bobolt can refresh the skills list mid-session:
...
- If you install plugins from npm (`bobolt plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.bobolt/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
  - Bobolt uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
bobolt pairing approve <channel> <code>
```
...
By default, Bobolt routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Bobolt has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.bobolt/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.bobolt or your logs.”
...
4. **Re-run audit**
   - `bobolt security audit --deep` and confirm the report is clean.
...
- `~/.bobolt/bobolt.json`: `600` (user read/write only)
- `~/.bobolt`: `700` (user only)

`bobolt doctor` can warn and offer to tighten these permissions.
...
The Gateway broadcasts its presence via mDNS (`_bobolt-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `bobolt doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Bobolt
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Bobolt verifies the identity by resolving the
...
- Bobolt will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.bobolt/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:

- `bobolt.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `bobolt status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@bobolt", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.bobolt/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `bobolt` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Bobolt clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `bobolt gateway` process.
...
1. Check Gateway logs: `/tmp/bobolt/bobolt-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.bobolt/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Bobolt version
...
Found a vulnerability in Bobolt? Please report responsibly:

1. Email: [security@bobolt.ai](mailto:security@bobolt.ai)
