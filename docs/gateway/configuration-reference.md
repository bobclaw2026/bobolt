...
description: "Complete field-by-field reference for ~/.bobolt/bobolt.json"
---

# Configuration Reference

Every field available in `~/.bobolt/bobolt.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Bobolt uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `bobolt doctor` into `whatsapp/default`.
...
Mattermost ships as a plugin: `bobolt plugins install @bobolt/mattermost`.
...
Bobolt spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@bobolt"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `bobolt.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Bobolt auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Bobolt assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.bobolt/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.bobolt/workspace",
        agentDir: "~/.bobolt/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@bobolt"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:bobolt`   | All built-in tools (excludes provider plugins)                                           |
...
Bobolt uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.bobolt/agents/<agentId>/agent/models.json`.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `bobolt onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `bobolt onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `bobolt onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `bobolt onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `bobolt onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `bobolt onboard --auth-choice minimax-api`.
...
- Loaded from `~/.bobolt/extensions`, `<workspace>/.bobolt/extensions`, plus `plugins.load.paths`.
...
Convenience flags: `--dev` (uses `~/.bobolt-dev` + port `19001`), `--profile <name>` (uses `~/.bobolt-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-bobolt-token: <token>`.
...
- Also serves A2UI at `/__bobolt__/a2ui/`.
...
- Hostname defaults to `bobolt`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.bobolt/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `bobolt dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.bobolt/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.bobolt/credentials/oauth.json`.
...
- Default log file: `/tmp/bobolt/bobolt-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `bobolt doctor --fix` can strip unknown keys).
