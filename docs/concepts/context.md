...
read_when:
  - You want to understand what “context” means in Bobolt
...
“Context” is **everything Bobolt sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Bobolt-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Bobolt builds the system prompt

The system prompt is **Bobolt-owned** and rebuilt each run. It includes:
...
By default, Bobolt injects a fixed set of workspace files (if present):
