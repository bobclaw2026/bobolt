...
- `/usage cost` in chats: local cost summary aggregated from Bobolt session logs.
- CLI: `bobolt status --usage` prints a full per-provider breakdown.
- CLI: `bobolt channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
