...
read_when:
  - Setting up Twitch chat integration for Bobolt
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Bobolt connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
bobolt plugins install @bobolt/twitch
```

Local checkout (when running from a git repo):

```bash
bobolt plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "bobolt", // Bot's Twitch account
