...
read_when:
  - You want Bobolt to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Bobolt to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`bobolt onboard`) and `bobolt channels add` list optional channel plugins.
...
```bash
bobolt plugins install @bobolt/nostr
```

Use a local checkout (dev workflows):

```bash
bobolt plugins install --link <path-to-bobolt>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "bobolt@example.com",
        "lud16": "bobolt@example.com"
      }
    }
  }
}
