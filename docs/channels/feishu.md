...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Bobolt to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
bobolt plugins install @bobolt/feishu
```

Local checkout (when running from a git repo):

```bash
bobolt plugins install ./extensions/feishu
```
...
If you just installed Bobolt, run the wizard:
...
2. Configuring app credentials in Bobolt
...
1. You already ran `bobolt channels add` for Feishu
2. The gateway is running (`bobolt gateway status`)
...
```bash
bobolt pairing approve feishu <CODE>
```
...
2. Run `bobolt logs --follow` and look for `chat_id`
...
2. Run `bobolt logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `bobolt gateway status`  | Show gateway status           |
| `bobolt gateway install` | Install/start gateway service |
| `bobolt gateway stop`    | Stop gateway service          |
| `bobolt gateway restart` | Restart gateway service       |
| `bobolt logs --follow`   | Tail gateway logs             |
...
4. Check logs: `bobolt logs --follow`
...
5. Ensure the gateway is running: `bobolt gateway status`
6. Check logs: `bobolt logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.bobolt/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.bobolt/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
