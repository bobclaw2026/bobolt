...
```bash
bobolt plugins install @bobolt/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
bobolt plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Bobolt" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `bobolt pairing list nextcloud-talk`
  - `bobolt pairing approve nextcloud-talk <CODE>`
