...
Tlon is a decentralized messenger built on Urbit. Bobolt connects to your Urbit ship and can
...
```bash
bobolt plugins install @bobolt/tlon
```

Local checkout (when running from a git repo):

```bash
bobolt plugins install ./extensions/tlon
```
...
Use these with `bobolt message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Bobolt replies in-thread.
