...
```bash
bobolt pairing list discord
bobolt pairing approve discord <CODE>
```
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in Bobolt config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces Bobolt auth and returns "not authorized".
...
- If command deploy/state is stale, restart gateway and re-check with `bobolt channels status --probe`.
