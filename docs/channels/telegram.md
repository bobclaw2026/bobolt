...
```
    ...
    - or read `chat.id` from `bobolt logs --follow`
...
- DM messages can carry `message_thread_id`; Bobolt routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    Bobolt can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Bobolt retries as plain text.
...
```
    ...
    - `~/.bobolt/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Bobolt enqueues system events like:
...
```bash
bobolt message send --channel telegram --target 123456789 --message "hi"
bobolt message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `bobolt channels status` warns when config expects unmentioned group messages.
    - `bobolt channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `bobolt logs --follow` for skip reasons
