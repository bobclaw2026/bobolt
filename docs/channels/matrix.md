...
Matrix is an open, decentralized messaging protocol. Bobolt connects as a Matrix **user**
...
```bash
bobolt plugins install @bobolt/matrix
```

Local checkout (when running from a git repo):

```bash
bobolt plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `bobolt plugins install @bobolt/matrix`
   - From a local checkout: `bobolt plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Bobolt calls the same
     login endpoint, stores the access token in `~/.bobolt/credentials/matrix/credentials.json`,
...
- On first connection, Bobolt requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Bobolt logs a warning.
...
`~/.bobolt/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `bobolt pairing list matrix`
  - `bobolt pairing approve matrix <CODE>`
...
- On startup, Bobolt resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
