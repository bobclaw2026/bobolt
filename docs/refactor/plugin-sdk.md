...
- Publish as `bobolt/plugin-sdk` (or export from core under `bobolt/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `BoboltPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: BoboltConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: BoboltConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: BoboltConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: BoboltConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: BoboltConfig): string;
  };
};
```
...
- Introduce `bobolt/plugin-sdk`.
- Add `api.runtime` to `BoboltPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `boboltRuntime: ">=2026.2.0"`).
