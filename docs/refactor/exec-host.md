...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.bobolt/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.bobolt/node.json` (node id + pairing token).
...
Path: `~/.bobolt/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.bobolt/exec-approvals.sock",
...
- Unix socket at `~/.bobolt/exec-approvals.sock` (0600).
