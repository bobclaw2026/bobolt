---
summary: "Use MiniMax M2.1 in Bobolt"
read_when:
  - You want MiniMax models in Bobolt
...
```bash
bobolt plugins enable minimax-portal-auth  # skip if already loaded.
bobolt gateway restart  # restart if gateway is already running
bobolt onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/bobolt/bobolt/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `bobolt.json`:
...
- Use `bobolt models list` and `bobolt models set minimax/MiniMax-M2.1` to switch.
...
- Running `bobolt configure` and selecting **MiniMax M2.1**, or
