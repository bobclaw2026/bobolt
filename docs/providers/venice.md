---
summary: "Use Venice AI privacy-focused models in Bobolt"
read_when:
  - You want privacy-focused inference in Bobolt
...
```bash
bobolt onboard --auth-choice venice-api-key
```
...
```bash
bobolt chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Bobolt shows all available Venice models. Pick based on your needs:
...
```bash
bobolt models set venice/claude-opus-45
bobolt models set venice/llama-3.3-70b
```

List all available models:

```bash
bobolt models list | grep venice
```

## Configure via `bobolt configure`

1. Run `bobolt configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Bobolt automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
bobolt chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
bobolt chat --model venice/claude-opus-45

# Use uncensored model
bobolt chat --model venice/venice-uncensored

# Use vision model with image
bobolt chat --model venice/qwen3-vl-235b-a22b

# Use coding model
bobolt chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `bobolt models list` to see currently available models. Some models may be temporarily offline.
