...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/bobolt`)
...
If the page fails to load, start the Gateway first: `bobolt gateway`.
...
you revoke it with `bobolt devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.bobolt/bobolt.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Bobolt
...
Bobolt **blocks** Control UI connections without device identity.
