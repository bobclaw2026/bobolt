...
```bash
bobolt tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `bobolt tui --deliver`
...
- Check the Gateway logs: `bobolt logs --follow`.
- Confirm the agent can run: `bobolt status` and `bobolt models status`.
...
- No agents in picker: check `bobolt agents list` and your routing config.
