...
你可以在 `~/.bobolt/bobolt.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/bobolt.log"
  }
}
```
...
```bash
bobolt channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.bobolt/bobolt.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/bobolt/bobolt-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Bobolt 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `bobolt plugins enable diagnostics-otel` 启用插件。
...
- `bobolt.tokens`（计数器，属性：`bobolt.token`、`bobolt.channel`、`bobolt.provider`、`bobolt.model`）
- `bobolt.cost.usd`（计数器，属性：`bobolt.channel`、`bobolt.provider`、`bobolt.model`）
- `bobolt.run.duration_ms`（直方图，属性：`bobolt.channel`、`bobolt.provider`、`bobolt.model`）
- `bobolt.context.tokens`（直方图，属性：`bobolt.context`、`bobolt.channel`、`bobolt.provider`、`bobolt.model`）

消息流：

- `bobolt.webhook.received`（计数器，属性：`bobolt.channel`、`bobolt.webhook`）
- `bobolt.webhook.error`（计数器，属性：`bobolt.channel`、`bobolt.webhook`）
- `bobolt.webhook.duration_ms`（直方图，属性：`bobolt.channel`、`bobolt.webhook`）
- `bobolt.message.queued`（计数器，属性：`bobolt.channel`、`bobolt.source`）
- `bobolt.message.processed`（计数器，属性：`bobolt.channel`、`bobolt.outcome`）
- `bobolt.message.duration_ms`（直方图，属性：`bobolt.channel`、`bobolt.outcome`）

队列 + 会话：

- `bobolt.queue.lane.enqueue`（计数器，属性：`bobolt.lane`）
- `bobolt.queue.lane.dequeue`（计数器，属性：`bobolt.lane`）
- `bobolt.queue.depth`（直方图，属性：`bobolt.lane` 或 `bobolt.channel=heartbeat`）
- `bobolt.queue.wait_ms`（直方图，属性：`bobolt.lane`）
- `bobolt.session.state`（计数器，属性：`bobolt.state`、`bobolt.reason`）
- `bobolt.session.stuck`（计数器，属性：`bobolt.state`）
- `bobolt.session.stuck_age_ms`（直方图，属性：`bobolt.state`）
- `bobolt.run.attempt`（计数器，属性：`bobolt.attempt`）

### 导出的 span（名称 + 关键属性）

- `bobolt.model.usage`
  - `bobolt.channel`、`bobolt.provider`、`bobolt.model`
  - `bobolt.sessionKey`、`bobolt.sessionId`
  - `bobolt.tokens.*`（input/output/cache_read/cache_write/total）
- `bobolt.webhook.processed`
  - `bobolt.channel`、`bobolt.webhook`、`bobolt.chatId`
- `bobolt.webhook.error`
  - `bobolt.channel`、`bobolt.webhook`、`bobolt.chatId`、`bobolt.error`
- `bobolt.message.processed`
  - `bobolt.channel`、`bobolt.outcome`、`bobolt.chatId`、`bobolt.messageId`、`bobolt.sessionKey`、`bobolt.sessionId`、`bobolt.reason`
- `bobolt.session.stuck`
  - `bobolt.state`、`bobolt.ageMs`、`bobolt.queueDepth`、`bobolt.sessionKey`、`bobolt.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `bobolt doctor`。
