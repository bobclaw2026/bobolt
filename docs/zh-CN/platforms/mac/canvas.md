...
- `~/Library/Application Support/Bobolt/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `bobolt-canvas://<session>/<path>`

示例：

- `bobolt-canvas://main/` → `<canvasRoot>/main/index.html`
- `bobolt-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `bobolt-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
bobolt nodes canvas present --node <id>
bobolt nodes canvas navigate --node <id> --url "/"
bobolt nodes canvas eval --node <id> --js "document.title"
bobolt nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__bobolt__/a2ui/
```
...
bobolt nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
bobolt nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "bobolt://agent?message=Review%20this%20design";
```
