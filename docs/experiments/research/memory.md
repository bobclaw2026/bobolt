...
read_when:
  - Designing workspace memory (~/.bobolt/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Bobolt integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.bobolt/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.bobolt/workspace` as canonical human-readable memory.
...
```
~/.bobolt/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Bobolt**, but keep a separable core library.

### Why integrate into Bobolt?

- Bobolt already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `bobolt memory recall "…" --k 25 --since 30d`
  - `bobolt memory reflect --since 7d`
...
Pragmatic take for `~/.bobolt/workspace`:
