...
bobolt system heartbeat last
bobolt logs --follow
bobolt config get agents.defaults.heartbeat
bobolt channels status --probe
```
...
```bash
bobolt config get agents.defaults.heartbeat.activeHours
bobolt config get agents.defaults.heartbeat.activeHours.timezone
bobolt config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
bobolt cron list
bobolt logs --follow
```
