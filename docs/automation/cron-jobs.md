...
- Jobs persist under `~/.bobolt/cron/` so restarts don’t lose schedules.
...
bobolt cron list
bobolt cron run <job-id>
bobolt cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.bobolt/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `bobolt cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Bobolt defaults to `announce`.
...
- Job store: `~/.bobolt/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.bobolt/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
bobolt cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
bobolt cron edit <jobId> --agent ops
bobolt cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
bobolt cron run <jobId>
bobolt cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
bobolt cron edit <jobId> \
...
```bash
bobolt system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`bobolt system event`](/cli/system).
...
- Bobolt applies exponential retry backoff for recurring jobs after consecutive errors:
