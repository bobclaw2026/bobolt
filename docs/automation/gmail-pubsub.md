---
summary: "Gmail Pub/Sub push wired into Bobolt webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to Bobolt
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> Bobolt

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> Bobolt webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- Bobolt hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the Bobolt helper to wire everything together (installs deps on macOS via brew):

```bash
bobolt webhooks gmail setup \
...
- Writes `hooks.gmail` config for `bobolt webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, Bobolt automatically sets
...
- `--hook-url` points to Bobolt `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to Bobolt.

Recommended: `bobolt webhooks gmail run` wraps the same flow and auto-renews the watch.
...
```bash
gog gmail watch status --account bobolt@gmail.com
gog gmail history --account bobolt@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account bobolt@gmail.com
