...
Bobolt has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
### Layer 2: Gateway + dev agent smoke (what “@bobolt” actually does)
...
```bash
bobolt models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: Bobolt calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: Bobolt shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `bobolt models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `bobolt models list` / model selection.

- Profile store: `~/.bobolt/credentials/` (preferred; what “profile keys” means in the tests)
...
- `OPENCLAW_CONFIG_DIR=...` (default: `~/.bobolt`) mounted to `/home/node/.bobolt`
- `OPENCLAW_WORKSPACE_DIR=...` (default: `~/.bobolt/workspace`) mounted to `/home/node/.bobolt/workspace`
