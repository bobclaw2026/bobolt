...
Bobolt features that can generate provider usage or paid API calls.
...
- `bobolt status --usage` and `bobolt channels list` show provider **usage windows**
...
Bobolt can pick up credentials from:
...
`bobolt models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
