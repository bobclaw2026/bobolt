...
Bobolt integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Bobolt owns lifecycle when `channels.signal.autoStart=true`.
...
- Bobolt spawns `imsg rpc` as a child process (legacy iMessage integration).
