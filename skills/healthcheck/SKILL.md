---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Bobolt deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Bobolt cron scheduling for periodic checks, or version status checks on a machine running Bobolt (laptop, workstation, Pi, VPS).
---

# Bobolt Host Hardening

## Overview

Assess and harden the host running Bobolt, then align it to a user-defined risk tolerance without breaking access. Use Bobolt security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Bobolt changes the host firewall, SSH, or OS updates; it does not.
...
5. Bobolt gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Bobolt can access sensitive data.
...
### 2) Run Bobolt security audits (read-only)

As part of the default read-only checks, run `bobolt security audit --deep`. Only offer alternatives if the user requests them:

1. `bobolt security audit` (faster, non-probing)
2. `bobolt security audit --json` (structured output)

Offer to apply Bobolt safe defaults (numbered):

1. `bobolt security audit --fix`

Be explicit that `--fix` only tightens Bobolt defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Bobolt version/update status (read-only)

As part of the default read-only checks, run `bobolt update status`.
...
- Credential hygiene notes (location of Bobolt creds, prefer disk encryption)
...
After Bobolt install or first hardening pass, run at least one baseline audit and version check:

- `bobolt security audit`
- `bobolt security audit --deep`
- `bobolt update status`

Ongoing monitoring is recommended. Use the Bobolt cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `bobolt cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `bobolt update status`
...
Before creating, `bobolt cron list` and match on exact `name`. If found, `bobolt cron edit <id> ...`.
If not found, `bobolt cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `bobolt update status` (preferred for source checkouts and channels)
2. `npm view bobolt version` (published npm version)

## Bobolt command accuracy

Use only supported commands and flags:

- `bobolt security audit [--deep] [--fix] [--json]`
- `bobolt status` / `bobolt status --deep`
- `bobolt health --json`
- `bobolt update status`
- `bobolt cron add|list|runs|run`

Do not invent CLI flags or imply Bobolt enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Bobolt compaction:
