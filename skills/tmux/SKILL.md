...
metadata:
  { "bobolt": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${OPENCLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/bobolt-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/bobolt.sock"
SESSION=bobolt-python
...
- Default socket path: `"$OPENCLAW_TMUX_SOCKET_DIR/bobolt.sock"`.
