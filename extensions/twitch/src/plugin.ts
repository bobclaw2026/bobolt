/**
 * Twitch channel plugin for Bobolt.
 *
...
import { buildChannelConfigSchema } from "bobolt/plugin-sdk";
import type {
 ...
 * for Bobolt. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: BoboltConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: BoboltConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: BoboltConfig): boolean => {
