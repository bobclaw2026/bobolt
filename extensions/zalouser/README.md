...
Bobolt extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
bobolt channels login --channel zalouser
...
```bash
bobolt message send --channel zalouser --target <threadId> --message "Hello from Bobolt!"
```
...
```bash
bobolt channels login --channel zalouser              # Login via QR
bobolt channels login --channel zalouser --account work
bobolt channels status --probe
bobolt channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
bobolt directory self --channel zalouser
bobolt directory peers list --channel zalouser --query "name"
bobolt directory groups list --channel zalouser --query "work"
bobolt directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
bobolt message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
bobolt message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `bobolt channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
bobolt channels login --channel zalouser --account work
bobolt message send --channel zalouser --account work --target <id> --message "Hello"
