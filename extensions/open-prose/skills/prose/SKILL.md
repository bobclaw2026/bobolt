...
metadata: { "bobolt": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Bobolt `sessions_spawn`
- **File I/O** == Bobolt `read`/`write`
- **Remote fetch** == Bobolt `web_fetch` (or `exec` with curl when POST is required)
