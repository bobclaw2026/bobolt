...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["boboltPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["bobolt", "pollId"]) ??
    readNestedString(value, ["bobolt", "poll", "id"]) ??
    readNestedString(value, ["data", "boboltPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "bobolt", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "bobolt poll vote",
          displayText: "Vote recorded",
          value: { boboltPollId: pollId, pollId },
        },
      },
    ],
  };
