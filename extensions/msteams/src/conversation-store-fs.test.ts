import type { PluginRuntime } from "bobolt/plugin-sdk";
...
const runtimeStub = {
  state: {
    resolveStateDir: (env: NodeJS.ProcessEnv = process.env, homedir?: () => string) => {
      ...
      return path.join(resolvedHome, ".bobolt");
    },
  },
...
describe("msteams conversation store (fs)", () => {
  ...
  it("filters and prunes expired entries (but keeps legacy ones)", async () => {
    const stateDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "bobolt-msteams-store-"));
