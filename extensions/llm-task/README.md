...
`bobolt.invoke --each`) without adding new Bobolt code per workflow.
...
This extension depends on Bobolt internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Bobolt extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.bobolt/extensions` as a standalone plugin directory.
