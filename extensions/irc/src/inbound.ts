...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as BoboltConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as BoboltConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as BoboltConfig);
