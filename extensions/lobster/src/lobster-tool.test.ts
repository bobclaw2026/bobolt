...
import type { BoboltPluginApi, BoboltPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "bobolt-lobster-plugin-") {
...
function fakeApi(overrides: Partial<BoboltPluginApi> = {}): BoboltPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<BoboltPluginToolContext> = {}): BoboltPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: BoboltPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
