import type { BoboltPluginApi, BoboltPluginService } from "bobolt/plugin-sdk";
...
function normalizeDenyList(cfg: BoboltPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: BoboltPluginApi["config"]): string[] {
...
export default function register(api: BoboltPluginApi) {
  ...
  const timerService: BoboltPluginService = {
