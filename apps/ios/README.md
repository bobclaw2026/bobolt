...
This is an **alpha** iOS app that connects to an Bobolt Gateway as a `role: node`.
...
3. On iOS: Bobolt → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Bobolt.xcodeproj -scheme Bobolt -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/BoboltKit` contains the shared transport/types used by the iOS app.
