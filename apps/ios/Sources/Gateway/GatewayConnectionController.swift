...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [BoboltCapability.canvas.rawValue, BoboltCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(BoboltCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(BoboltCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = BoboltLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(BoboltCapability.location.rawValue) }

        caps.append(BoboltCapability.device.rawValue)
        caps.append(BoboltCapability.photos.rawValue)
        caps.append(BoboltCapability.contacts.rawValue)
        caps.append(BoboltCapability.calendar.rawValue)
        caps.append(BoboltCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(BoboltCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            BoboltSystemCommand.notify.rawValue,
            BoboltChatCommand.push.rawValue,
        ...
        if caps.contains(BoboltCapability.camera.rawValue) {
            commands.append(BoboltCameraCommand.list.rawValue)
            commands.append(BoboltCameraCommand.snap.rawValue)
            commands.append(BoboltCameraCommand.clip.rawValue)
        }
        if caps.contains(BoboltCapability.location.rawValue) {
            commands.append(BoboltLocationCommand.get.rawValue)
        }
        if caps.contains(BoboltCapability.device.rawValue) {
            commands.append(BoboltDeviceCommand.status.rawValue)
            commands.append(BoboltDeviceCommand.info.rawValue)
        }
        if caps.contains(BoboltCapability.photos.rawValue) {
            commands.append(BoboltPhotosCommand.latest.rawValue)
        }
        if caps.contains(BoboltCapability.contacts.rawValue) {
            commands.append(BoboltContactsCommand.search.rawValue)
            commands.append(BoboltContactsCommand.add.rawValue)
        }
        if caps.contains(BoboltCapability.calendar.rawValue) {
            commands.append(BoboltCalendarCommand.events.rawValue)
            commands.append(BoboltCalendarCommand.add.rawValue)
        }
        if caps.contains(BoboltCapability.reminders.rawValue) {
            commands.append(BoboltRemindersCommand.list.rawValue)
            commands.append(BoboltRemindersCommand.add.rawValue)
        }
        if caps.contains(BoboltCapability.motion.rawValue) {
            commands.append(BoboltMotionCommand.activity.rawValue)
            commands.append(BoboltMotionCommand.pedometer.rawValue)
        }
