...
final class ContactsService: ContactsServicing {
    ...
    func search(params: BoboltContactsSearchParams) async throws -> BoboltContactsSearchPayload {
        ...
        return BoboltContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: BoboltContactsAddParams) async throws -> BoboltContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return BoboltContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return BoboltContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> BoboltContactPayload {
        BoboltContactPayload(
