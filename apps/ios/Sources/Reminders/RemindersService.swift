...
final class RemindersService: RemindersServicing {
    func list(params: BoboltRemindersListParams) async throws -> BoboltRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[BoboltReminderPayload], Error>) in
        ...
        return BoboltRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: BoboltRemindersAddParams) async throws -> BoboltRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = BoboltReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return BoboltRemindersAddPayload(reminder: payload)
    }
