...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<BoboltTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = BoboltTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> BoboltTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return BoboltTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> BoboltTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> BoboltTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return BoboltTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> BoboltTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = BoboltTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: BoboltTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: BoboltAgentEventPayload.self) else {
                continue
            }
