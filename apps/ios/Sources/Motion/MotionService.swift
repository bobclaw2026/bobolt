...
final class MotionService: MotionServicing {
    func activities(params: BoboltMotionActivityParams) async throws -> BoboltMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[BoboltMotionActivityEntry], Error>) in
        ...
        return BoboltMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: BoboltPedometerParams) async throws -> BoboltPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<BoboltPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = BoboltPedometerPayload(
