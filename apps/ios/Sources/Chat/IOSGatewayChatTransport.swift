...
struct IOSGatewayChatTransport: BoboltChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> BoboltChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(BoboltChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> BoboltChatHistoryPayload {
        ...
        return try JSONDecoder().decode(BoboltChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [BoboltChatAttachmentPayload]) async throws -> BoboltChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [BoboltChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(BoboltChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(BoboltGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: BoboltGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: BoboltChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: BoboltAgentEventPayload.self)
                        {
