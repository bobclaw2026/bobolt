...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: BoboltProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: BoboltProtocol.AnyCodable] = [
            "presence": .init([BoboltProtocol.AnyCodable(entry)]),
        ]
        let payload = BoboltProtocol.AnyCodable(payloadMap)
