...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.bobolt", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = BoboltConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.boboltExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "bobolt CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "bobolt@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "bobolt@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "bobolt@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "bobolt@\(target)"])
            }

        statusHandler("Installing bobolt@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed bobolt@\(target)")
        } else {
