...
private let webChatSwiftLogger = Logger(subsystem: "ai.bobolt", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: BoboltChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> BoboltChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(BoboltChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> BoboltChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                BoboltGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<BoboltChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any BoboltChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = BoboltChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: BoboltChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<BoboltChatView>) -> NSViewController
    {
