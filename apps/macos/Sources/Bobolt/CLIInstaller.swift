...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("bobolt").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing bobolt CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed bobolt \($0)." } ?? "Installed bobolt."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://bobolt.bot/install-cli.sh | \
