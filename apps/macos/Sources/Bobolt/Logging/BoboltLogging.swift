...
enum BoboltLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = BoboltOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = BoboltFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        BoboltLogging.bootstrapIfNeeded()
        let label = BoboltLogging.makeLabel(subsystem: subsystem, category: category)
...
struct BoboltOSLogHandler: LogHandler {
...
struct BoboltFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = BoboltLogging.parseLabel(self.label)
