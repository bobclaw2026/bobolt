...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: BoboltProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.bobolt", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: BoboltKit.AnyCodable]()) {
                $0[$1.key] = BoboltKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: BoboltProtocol.AnyCodable?) -> [String: BoboltProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: BoboltProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: BoboltKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: BoboltProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("bobolt.control.heartbeat")
    static let controlAgentEvent = Notification.Name("bobolt.control.agent")
}
