...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: BoboltOAuthStore.AnthropicOAuthStatus = BoboltOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: BoboltOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = BoboltOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = BoboltOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try BoboltOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Bobolt can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: BoboltOAuthStore.AnthropicOAuthStatus,
