...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.bobolt", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: BoboltPeekabooBridgeServices?
    private static var boboltSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Bobolt", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = BoboltPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.boboltSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.boboltSocketPath, privacy: .public)")
    }
...
@MainActor
private final class BoboltPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.bobolt.peekaboo")
