...
enum DebugActions {
    private static let verboseDefaultsKey = "bobolt.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "bobolt.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = BoboltPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Bobolt", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = BoboltPaths.configURL
        guard
