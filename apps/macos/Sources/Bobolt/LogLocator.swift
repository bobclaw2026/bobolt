...
enum LogLocator {
    private static var logDir: URL {
        ...
        let preferred = URL(fileURLWithPath: "/tmp/bobolt")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("bobolt-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("bobolt-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/bobolt/ (rolling or stdout), or nil if none exist.
