...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.bobolt", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> BoboltSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: BoboltSessionPreviewEntry,
    ...
    private static func cache(payload: BoboltSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: BoboltChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [BoboltKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [BoboltKit.AnyCodable]) -> [BoboltChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(BoboltChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: BoboltChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: BoboltChatMessage) -> Bool {
    ...
    private static func toolNames(for message: BoboltChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: BoboltChatMessage) -> String? {
