...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Bobolt token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: BoboltOAuthStore.AnthropicOAuthStatus = BoboltOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.bobolt", category: "anthropic-oauth")
...
enum BoboltOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let boboltOAuthDirEnv = "OPENCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Bobolt OAuth token file not found"
            case .unreadableFile: "Bobolt OAuth token file not readable"
            case .invalidJSON: "Bobolt OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Bobolt OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Bobolt OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.boboltOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".bobolt", isDirectory: true)
