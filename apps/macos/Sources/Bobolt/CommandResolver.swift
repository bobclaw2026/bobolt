...
enum CommandResolver {
    private static let projectRootDefaultsKey = "bobolt.gatewayProjectRootPath"
    private static let helperName = "bobolt"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let boboltEntry = root.appendingPathComponent("bobolt.mjs").path
        if FileManager().isReadableFile(atPath: boboltEntry) { return boboltEntry }
        let binEntry = root.appendingPathComponent("bin/bobolt.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/bobolt")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let boboltPaths = self.boboltManagedPaths(home: home)
        if !boboltPaths.isEmpty {
            extras.insert(contentsOf: boboltPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + boboltPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func boboltManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".bobolt"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func boboltExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectBoboltExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("bobolt.mjs").path,
            root.appendingPathComponent("bin/bobolt.js").path,
        ]
    ...
    static func hasAnyBoboltInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.boboltExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let boboltPath = self.projectBoboltExecutable(projectRoot: root) {
                return [boboltPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "bobolt", subcommand] + extraArgs
            }
            if let boboltPath = self.boboltExecutable(searchPaths: searchPaths) {
                return [boboltPath, subcommand] + extraArgs
            }

            let missingEntry = """
            bobolt entrypoint missing (looked for dist/index.js or bobolt.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real bobolt CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/bobolt"
        ...
        \(cliSection)
        \(projectSection)
        if command -v bobolt >/dev/null 2>&1; then
          CLI="$(command -v bobolt)"
          bobolt \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bobolt.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bobolt.mjs"
            node "$PRJ/bobolt.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/bobolt.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/bobolt.js"
            node "$PRJ/bin/bobolt.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent bobolt"
          pnpm --silent bobolt \(quotedArgs);
        else
          echo "bobolt CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? BoboltConfigFile.loadDict()
