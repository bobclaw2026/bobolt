...
private final class TestChatTransport: @unchecked Sendable, BoboltChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [BoboltChatHistoryPayload]
    private let sessionsResponses: [BoboltChatSessionsListResponse]

    private let stream: AsyncStream<BoboltChatTransportEvent>
    private let continuation: AsyncStream<BoboltChatTransportEvent>.Continuation

    init(
        historyResponses: [BoboltChatHistoryPayload],
        sessionsResponses: [BoboltChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<BoboltChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> BoboltChatHistoryPayload {
        ...
        return self.historyResponses.last ?? BoboltChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [BoboltChatAttachmentPayload]) async throws -> BoboltChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return BoboltChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> BoboltChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? BoboltChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: BoboltChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = BoboltChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = BoboltChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { BoboltChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = BoboltChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = BoboltChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = BoboltChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { BoboltChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
