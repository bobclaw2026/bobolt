...
@MainActor
struct BoboltChatComposer: View {
    @Bindable var viewModel: BoboltChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(BoboltChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(BoboltChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \BoboltPendingAttachment.id)
                { (att: BoboltPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            BoboltPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(BoboltChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(BoboltChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(BoboltChatTheme.subtleCard)
        .clipShape(Capsule())
    }
