...
private let chatUILogger = Logger(subsystem: "ai.bobolt", category: "BoboltChatUI")

@MainActor
@Observable
public final class BoboltChatViewModel {
    public private(set) var messages: [BoboltChatMessage] = []
    ...
    public var attachments: [BoboltPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [BoboltChatPendingToolCall] = []
    public private(set) var sessions: [BoboltChatSessionEntry] = []
    private let transport: any BoboltChatTransport
    ...
    private var pendingToolCallsById: [String: BoboltChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any BoboltChatTransport) {
    ...
    public var sessionChoices: [BoboltChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [BoboltChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [BoboltChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: BoboltPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [BoboltChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: BoboltChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [BoboltChatMessage]) -> [BoboltChatMessage] {
        var result: [BoboltChatMessage] = []
    ...
    private static func dedupeKey(for message: BoboltChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [BoboltChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> BoboltChatAttachmentPayload in
            BoboltChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> BoboltChatSessionEntry {
        BoboltChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: BoboltChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: BoboltChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: BoboltAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = BoboltChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> BoboltPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
