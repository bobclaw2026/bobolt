...
IMAGE_NAME="${OPENCLAW_IMAGE:-${CLAWDBOT_IMAGE:-bobolt:local}}"
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${CLAWDBOT_CONFIG_DIR:-$HOME/.bobolt}}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-${CLAWDBOT_WORKSPACE_DIR:-$HOME/.bobolt/workspace}}"
...
docker run --rm -t \
  ...
  -v "$CONFIG_DIR":/home/node/.bobolt \
  -v "$WORKSPACE_DIR":/home/node/.bobolt/workspace \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
