...
BASE_IMAGE="${BASE_IMAGE:-bobolt-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-bobolt-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=bobolt.sandbox=1)
NOTE
