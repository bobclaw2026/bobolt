#!/usr/bin/env bash
# ClawDock - Docker helpers for Bobolt
# Inspired by Simon Willison's "Running Bobolt in Docker"
# https://til.simonwillison.net/llms/bobolt-docker
#
# Installation:
...
# Common paths to check for Bobolt
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/bobolt"
  "${HOME}/workspace/bobolt"
  "${HOME}/projects/bobolt"
  "${HOME}/dev/bobolt"
  "${HOME}/code/bobolt"
  "${HOME}/src/bobolt"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Bobolt at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/bobolt"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Bobolt not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/bobolt/bobolt.git ~/bobolt"
    echo "  cd ~/bobolt && ./docker-setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d bobolt-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart bobolt-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f bobolt-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec bobolt-gateway \
    bash -c 'echo "alias bobolt=\"./bobolt.mjs\"" > /tmp/.bashrc_bobolt && bash --rcfile /tmp/.bashrc_bobolt'
}
...
clawdock-cli() {
  _clawdock_compose run --rm bobolt-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build bobolt-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" bobolt-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" bobolt-gateway \
    bash -c './bobolt.mjs config set gateway.remote.token "$TOKEN" && ./bobolt.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec bobolt-gateway \
    bash -c "./bobolt.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart bobolt-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm bobolt-cli dashboard --no-open 2>&1)
  status=$?
...
clawdock-devices() {
  ...
  local output status
  output=$(_clawdock_compose exec bobolt-gateway node dist/index.js devices list 2>&1)
  status=$?
  ...
  if [ $status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'bobolt config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Bobolt${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (bobolt alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to bobolt project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.bobolt)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'bobolt channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'bobolt status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.bobolt.ai${_CLR_RESET}"
  echo ""
}
