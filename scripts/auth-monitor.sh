...
#
# Suggested cron: */30 * * * * /home/admin/bobolt/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Bobolt notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., bobolt-alerts)
...
STATE_FILE="$HOME/.bobolt/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Bobolt if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use bobolt
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Bobolt to $NOTIFY_PHONE..."
            bobolt send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Bobolt is down. Run: ssh l36 '~/bobolt/scripts/mobile-reauth.sh'" "urgent"
    exit 1
