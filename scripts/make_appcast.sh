...
ZIP=${1:?"Usage: $0 Bobolt-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/bobolt/bobolt/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Bobolt-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/bobolt/bobolt/releases/download/v${VERSION}/"}
