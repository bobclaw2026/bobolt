#!/bin/bash
# Setup Bobolt Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'bobolt-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Bobolt message: Send warning via Bobolt itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/bobolt-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/bobolt-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/bobolt-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now bobolt-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status bobolt-auth-monitor.timer"
echo ""
echo "Quick commands:"
