...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/bobolt-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/bobolt-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/bobolt-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/bobolt-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    workspace_dir="$HOME/.bobolt/workspace"
    config_path="$HOME/.bobolt/bobolt.json"
    sessions_dir="$HOME/.bobolt/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.bobolt/bobolt.json" <<'"'"'JSON'"'"'
{
