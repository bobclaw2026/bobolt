...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/bobolt/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Bobolt Re-Auth" -c "After getting key, run: ssh $SERVER '~/bobolt/scripts/mobile-reauth.sh'" --id bobolt-auth
        ;;
    *)
