 ...
 * `bobolt update`. Use it to verify the completion installation flow
...
\`bobolt update\`, \`bobolt doctor\`, and \`bobolt onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Bobolt Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`bobolt completion --install\` later to enable.`));
    return;
  }
