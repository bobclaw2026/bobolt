...
This document describes how the Bobolt ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`bobolt acp` exposes an ACP agent over stdio and forwards prompts to a running
Bobolt Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Bobolt Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `bobolt acp` over stdio.

Example config:

```bash
bobolt config set gateway.remote.url wss://gateway-host:18789
bobolt config set gateway.remote.token <token>
```

Example run:

```bash
bobolt acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
bobolt acp --session agent:main:main
bobolt acp --session agent:design:main
bobolt acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Bobolt ACP” to start a thread.

## Execution Model

- ACP client spawns `bobolt acp` and speaks ACP messages over stdio.
...
```bash
bobolt acp --session agent:main:main
bobolt acp --session-label "support inbox"
bobolt acp --reset-session
```
...
`bobolt acp` resolves the Gateway URL and auth from CLI flags or config:
